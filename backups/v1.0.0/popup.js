...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  async function loadTaxonomyAndSettings() {
    ...
    const rawTags = syncData.quick_tags || syncData.tagsilo_tags || localData.quick_tags || localData.tagsilo_tags || [
      "High Priority",
      "Executive",
      "Warm Intro",
      "Founder",
    ...
    quickTags = rawTags.map(cleanTag);
  ...
  // Cross-Browser Google Authorization (Native getAuthToken with silent auto-renewal & launchWebAuthFlow fallback)
  async function authenticateWithGoogle(interactive = true) {
    ...
    // 2. Fallback to launchWebAuthFlow if interactive mode is requested
    if (interactive) {
    ...
    throw new Error("Silent Google authentication refresh could not complete.");
  }
  ...
  primarySyncBtn.addEventListener("click", async () => {
    ...
    // 3. Ensure Google Auth Token is active with silent auto-refresh
    if (!currentAuthToken) {
      try {
        ...
        // Prompt interactively only if silent refresh is impossible
        try {
