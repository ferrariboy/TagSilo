...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // 1. Google OAuth Identity Controller (Unified with popup.js)
  async function authenticateWithGoogle(interactive = true) {
    ...
    try {
      ...
      console.warn("[TagSilo Options] Native getAuthToken note:", nativeErr.message);
      if (!interactive) {
        throw nativeErr;
      }
    }

    // 2. Fallback to launchWebAuthFlow if interactive mode
    if (interactive) {
